...
async def submit_answer(request_data: dict):
    """Submit answer - passes stored coding test results to evaluator"""
    try:
        ...
        if test_session:
            ...
            if coding_results:
                logger.info(
                    ...
                    f"{coding_results.get('overall_result','?')} "
        ...
        if response.test_completed:
            ...
            return {
                ...
                "currentSection":        current_section,
                "sectionProgress":       section_progress,
                "sectionJustCompleted":  getattr(response, 'section_just_completed', None),
                "nextSectionStarting":   getattr(response, 'next_section_starting', None),
                # snake_case
...
async def regenerate_pdf(test_id: str):
    try:
        ...
        if not results:
            raise HTTPException(status_code=404, detail="Test results not found")
        pdf_bytes = await pdf_service.generate_test_results_pdf(test_id)
        return {"success": True, "message": "PDF regenerated successfully",
                "testId": test_id, "test_id": test_id, "pdfAvailable": True}
    except HTTPException:
        raise
    except Exception as e:
    ...
    try:
        ...
        if test_id and question_number:
            ...
            if test_session:
                ...
                if q:
                    q["user_code"]     = code
                    q["user_language"] = language
                logger.info(
                    f"💾 [run-tests] Stored results for Q{question_number}: "
...
async def get_test_cases_route(test_id: str, question_number: int):
    try:
        ...
        questions = test_session.get("questions", [])
        question  = next((q for q in questions if q.get("question_number") == question_number), None)
...
async def submit_code_route(request_data: dict):
    ...
    Stores results in session so /api/test/submit can use them for scoring.
    """
    try:
        test_id         = request_data.get("test_id")
        question_number = request_data.get("question_number")
...
async def get_warning_types():
    return {
        "warning_types": [
            ...
            {"type": "face_multiple",      "description": "Multiple faces detected"},
            {"type": "face_looking_away",  "description": "User looking away from screen"},
            {"type": "object_phone",       "description": "Mobile phone detected"},
            {"type": "object_book",        "description": "Book or reading material detected"},
            {"type": "object_person",      "description": "Multiple persons detected"},
            {"type": "tab_switch",         "description": "Tab or window switching detected"},
            {"type": "right_click",        "description": "Right-click attempted"},
            {"type": "low_light",          "description": "Low lighting conditions"},
            {"type": "multiple_faces",     "description": "Multiple faces (legacy)"},
            {"type": "object_detected",    "description": "Suspicious object (legacy)"},
...
async def get_student_history(student_id: str):
    try:
        ...
        return {"studentId": student_id, "student_id": student_id,
                "count": len(formatted), "history": formatted}
    except Exception as e:
        logger.error(f"❌ Get student history failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/api/test/user-type/{student_id}")
async def get_user_type(student_id: str):
    """Get user type (dev/non_dev) for a student from MySQL"""
    try:
        profile = _get_student_profile(int(student_id))
        if not profile:
            raise HTTPException(status_code=404, detail=f"Student {student_id} not found")
        user_type = profile.get("user_type")
        if not user_type:
            raise HTTPException(status_code=404, detail=f"Student {student_id} has no Role_Type set")
        return {"user_type": user_type, "student_id": student_id}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"❌ get_user_type failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_specific_question(test_id: str, question_number: int):
    try:
        ...
        return {
            "success":         True,
            "questionNumber":  question_number,  "question_number": question_number,
            "totalQuestions":  total_questions,  "total_questions": total_questions,
            "questionHtml":    question_html,
            "questionType":    question_type,    "question_type": question_type,
            "title":           question.get("title", ""),
            "options":         options,
            "isMcq":           is_mcq,           "is_mcq": is_mcq,
            "timeLimit":       time_limit,       "time_limit": time_limit,
            "savedAnswer":     saved_answer,     "saved_answer": saved_answer,
            "sectionInfo":     section_info,     "section_info": section_info,
            "currentSection":  current_section,  "current_section": current_section,
            "sectionProgress": section_progress, "section_progress": section_progress,
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def get_exam_info():
    return {
        "developer": {
            ...
            "total_questions": 25, "total_time_minutes": 62
        },
        "non_developer": {
            "sections": [
